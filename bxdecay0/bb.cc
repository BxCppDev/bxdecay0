...
namespace bxdecay0 {
  ...
  label_1:
    ...
    if (modebb == LEGACY_MODEBB_1 || modebb == LEGACY_MODEBB_2 || modebb == LEGACY_MODEBB_3 || modebb == LEGACY_MODEBB_7
        || modebb == LEGACY_MODEBB_17 || modebb == LEGACY_MODEBB_18) {
    ...
    if (modebb == LEGACY_MODEBB_7) {
      double w1 = e1 + emass;
      double w2 = e2 + emass;
      a         = 5. * (w1 * w2 + emass2) * (gsl_pow_2(p1) + gsl_pow_2(p2)) - gsl_pow_2(p1 * p2);
    ...
    if (modebb == LEGACY_MODEBB_18) {
      ...
      double a5   = -8. * (rksi * (et1 * et2 + 1.) - 2. * r * et0) / (3. * gsl_pow_2(r) * et1 * et2);
      double a6   = 2. * ((gsl_pow_2(rksi) + 4. * gsl_pow_2(r)) * (et1 * et2 + 1.) - 4. * rksi * r * et0)
      ...
      double b_eta = gsl_pow_2(et1 - et2) * gsl_pow_2(chi_2plus) / 2. - 4. * gsl_pow_2(chi_1minus) / 81.
                     + 8. * gsl_pow_2(rksi * chip_P / 6. - chip_R) / gsl_pow_2(r) - 8. * std::pow(chip_P, (2 / 9.));
      if (a_eta != 0.) {
        b = b_eta / a_eta * b1 * b2;
      }
    }
