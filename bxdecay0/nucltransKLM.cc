...
namespace bxdecay0 {
  ...
  void decay0_nucltransKLM(i_random & prng_,
                           event & event_,
                           ...
                           const double EbindeM_,
                           const double conveM_,
                           const double convp_,
                           const double tclev_,
                           const double thlev_,
                           double & tdlev_)
  {
    ...
    bool debug          = false;
    if (debug)
      std::cerr << "genbb::decay0::decay0_nucltransKLM: Egamma_=" << Egamma_ << std::endl;
    if (p <= 1.) {
      if (debug)
        std::cerr << "genbb::decay0::decay0_nucltransKLM: "
                  << "gamma" << std::endl;
