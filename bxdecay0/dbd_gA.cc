...
namespace bxdecay0 {
  ...
  void dbd_gA::set_nuclide(const std::string & nuclide_)
  {
    if (!is_nuclide_supported(nuclide_) and nuclide_ != "Test") {
      throw std::logic_error("bxdecay0::dbd_gA::set_nuclide: Unsupported nuclide '" + nuclide_ + "'!");
    }
    _nuclide_ = nuclide_;
    return;
  }
  ...
  void dbd_gA::_load_tabulated_pdf_()
  {
    ...
    if (!f_tab_pdf) {
      throw std::logic_error("bxdecay0::dbd_gA::_load_tabulated_pdf_: Cannot open file '" + _tabulated_prob_file_path_
                             + "'!");
    }
    ...
    while (f_tab_pdf) {
      ...
      if (!parsed_esum) {
        ...
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: esum = " << _pimpl_->tab_prob.esum_max
                    << std::endl;
        }
        if (!line_in) {
      ...
      if (!parsed_energy_sampling_header) {
        ...
        if (sampling_label != "Probability") {
          throw std::logic_error("bxdecay0::dbd_gA::_load_tabulated_pdf_: Unexpected probabibilty sampling '"
                                 + sampling_label + "'!");
        }
        ...
        line_in >> _pimpl_->tab_prob.e_min[0] >> _pimpl_->tab_prob.e_max[0] >> _pimpl_->tab_prob.energy_step
            >> _pimpl_->tab_prob.nsamples;
        if (!line_in) {
          throw std::logic_error("bxdecay0::dbd_gA::_load_tabulated_pdf_: Format error at line #"
                                 + std::to_string(nlines) + "!");
        }
        if (_pimpl_->tab_prob.e_min[0] < 0.0 or _pimpl_->tab_prob.e_min[0] >= _pimpl_->tab_prob.e_max[0]) {
        ...
        _pimpl_->tab_prob.energy_step
            = (_pimpl_->tab_prob.e_max[0] - _pimpl_->tab_prob.e_min[0]) / (_pimpl_->tab_prob.nsamples - 1);
        ...
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: e_min[0] = " << _pimpl_->tab_prob.e_min[0]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: e_max[0] = " << _pimpl_->tab_prob.e_max[0]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf : e_min[1] = " << _pimpl_->tab_prob.e_min[1]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: e_max[1] = " << _pimpl_->tab_prob.e_max[1]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: energy_step = " << _pimpl_->tab_prob.energy_step
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: nsamples = " << _pimpl_->tab_prob.nsamples
                    << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: # of energy samples  = "
                    << _pimpl_->tab_prob.energies.size() << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: min sampled energy   = "
                    << _pimpl_->tab_prob.energies.front() << " MeV" << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: max sampled energy   = "
                    << _pimpl_->tab_prob.energies.back() << " MeV" << std::endl;
        }
      ...
      // Load/parse a line of probabilities:
      {
        ...
        while (line_in and !line_in.eof()) {
          ...
          if (prob < 0.0) {
            throw std::logic_error("bxdecay0::dbd_gA::_load_tabulated_pdf_: Invalid p.d.f. value ["
                                   + std::to_string(prob) + "] at line #" + std::to_string(nlines) + "!");
          }
          e2_sample_count++;
          ...
          if (e1 + e2 > _pimpl_->tab_prob.esum_max) {
            if (prob > 0.0) {
              throw std::logic_error("bxdecay0::dbd_g::_load_tabulated_pdf_: Invalid p.d.f. value ["
                                     + std::to_string(prob) + "] at line #" + std::to_string(nlines)
                                     + "! Should be zero!");
            }
          }
        ...
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: expected  E2 cprob count = "
                    << e2_expected_samples << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: effective E2 cprob count = " << e2_sample_count
                    << std::endl;
        }
        if (e2_sample_count != e2_expected_samples) {
          throw std::logic_error(
              "bxdecay0::dbd_gA::_load_tabulated_pdf_: expected vs effective E2 prob count match issue!");
        }
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_pdf_: E2 pdf count #" << e2_pdf_count << " has "
                    << e2_sample_count << " p.d.f. samples for E2" << std::endl;
        }
        e2_pdf_count++;
      }
      f_tab_pdf >> std::ws;
  ...
  void dbd_gA::_load_tabulated_cdf_opt_()
  {
    ...
    debug      = false;
    if (debug) {
      std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: Loading c.d.f. file = '"
                << _tabulated_prob_file_path_ << "'" << std::endl;
    }
    ...
    while (f_tab_cdf) {
      ...
      nlines++;
      // if (debug) {
      //   std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: raw_line = <" << raw_line << '>' <<
      //   std::endl;
      // }
      if (raw_line.size() == 0) {
        if (debug) {
      ...
      if (!parsed_esum) {
        ...
        line_in >> _pimpl_->tab_prob.esum_max;
        if (debug) {
          std::cerr << "[devel] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: esum = " << _pimpl_->tab_prob.esum_max
                    << std::endl;
        }
        if (!line_in) {
      ...
      if (!parsed_energy_sampling_header) {
        ...
        line_in >> sampling_label;
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: sampling mode " << sampling_label
                    << std::endl;
        }

        if (sampling_label != "CumulativeProbability") {
          throw std::logic_error("bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: Unexpected probabibilty mode '"
                                 + sampling_label + "'!");
        }
        ...
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: e_min[0] = " << _pimpl_->tab_prob.e_min[0]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: e_max[0] = " << _pimpl_->tab_prob.e_max[0]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: e_min[1] = " << _pimpl_->tab_prob.e_min[1]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: e_max[1] = " << _pimpl_->tab_prob.e_max[1]
                    << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: energy_step  = "
                    << _pimpl_->tab_prob.energy_step << " MeV " << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: nsamples = " << _pimpl_->tab_prob.nsamples
                    << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: # of energy samples  = "
                    << _pimpl_->tab_prob.energies.size() << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: min sampled energy   = "
                    << _pimpl_->tab_prob.energies.front() << " MeV" << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: max sampled energy   = "
                    << _pimpl_->tab_prob.energies.back() << " MeV" << std::endl;
        }
        parsed_energy_sampling_header = true;
        continue;
      ...
      if (!parsed_e1_cdf) {
        ...
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: # of energy samples  = "
                    << _pimpl_->tab_prob.e1_cprobs.size() << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: min sampled cprob   = "
                    << _pimpl_->tab_prob.e1_cprobs.front() << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: max sampled cprob   = "
                    << _pimpl_->tab_prob.e1_cprobs.back() << std::endl;
        }
        continue;
      }
      ...
      // Load/parse a line of E2 cumulative probabilities per E1 sample:
      {
        ...
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: expected  E2 cprob count  = "
                    << e2_expected_samples << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: effective E2 cprob count = "
                    << cdf_probs.size() << std::endl;
        }
        if (cdf_probs.size() != e2_expected_samples) {
          throw std::logic_error(
              "bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: expected vs effective E2 cprob count match issue!");
        }
        e2_cdf_count++;
        if (debug) {
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: E2 cdf count #" << e2_cdf_count << " has "
                    << cdf_probs.size() << " c.d.f. samples for E2" << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_:  - min cumul prob = " << cdf_probs.front()
                    << std::endl;
          std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_:  - max cumul prob = " << cdf_probs.back()
                    << std::endl;
        }
      }
    ...
    if (debug) {
      ...
      std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: Maximum sampled energy = "
                << std::to_string(_pimpl_->tab_prob.e_max[0]) << " MeV" << std::endl;
      std::cerr << "[debug] bxdecay0::dbd_gA::_load_tabulated_cdf_opt_: Cumulative probability sampling is parsed."
                << std::endl;
    }
    return;
  }
  ...
  void dbd_gA::print(std::ostream & out_, const std::string & title_, const std::string & indent_) const
  {
    if (!title_.empty()) {
      out_ << indent_ << title_ << "\n";
    }
    ...
    if (is_initialized()) {
      if (_shooting_ == SHOOTING_REJECTION) {
        ...
        for (int i = 0; i < 2; i++) {
          out_ << indent_ << skip_tag << tag << "e" << i + 1 << "(nsamples) = " << _pimpl_->tab_prob.e_nsamples[i]
               << std::endl;
          out_ << indent_ << skip_tag << tag << "e" << i + 1 << "(min) = " << _pimpl_->tab_prob.e_min[i] << " MeV"
               << std::endl;
          out_ << indent_ << skip_tag << tag << "e" << i + 1 << "(max) = " << _pimpl_->tab_prob.e_max[i] << " MeV"
               << std::endl;
        }
        out_ << indent_ << skip_tag << last_tag << "Prob(max) = " << _pimpl_->tab_prob.prob_max << std::endl;
        out_ << indent_ << tag << "P.d.f Interpolator : " << std::endl;
        out_ << indent_ << skip_tag << last_tag << "Name : '" << gsl_interp2d_name(_pimpl_->pdf_interpolator.interp)
             << "'" << std::endl;
      } else if (_shooting_ == SHOOTING_INVERSE_TRANSFORM_METHOD) {
        ...
        out_ << indent_ << skip_tag << tag << "# e1/e2 energy prob samples = " << _pimpl_->tab_prob.energies.size()
             << std::endl;
        out_ << indent_ << skip_tag << tag << "e1/e2 energy_step           = " << _pimpl_->tab_prob.energy_step
             << " MeV" << std::endl;
        out_ << indent_ << skip_tag << tag
             << "# c.d.f. prob e1 sampled energies = " << _pimpl_->tab_prob.e1_cprobs.size() << std::endl;
        out_ << indent_ << skip_tag << last_tag
             << "# c.d.f. prob e2 sampled energies = " << _pimpl_->tab_prob.e2_cprobs.size() << std::endl;
      }
    }
  ...
  void dbd_gA::_shoot_e1_e2_rejection_(i_random & prng_, double & e1_, double & e2_)
  {
    while (true) {
      ...
      double e1 = _pimpl_->tab_prob.e_samples[0].front()
                  + r1 * (_pimpl_->tab_prob.e_samples[0].back() - _pimpl_->tab_prob.e_samples[0].front());
      double e2 = _pimpl_->tab_prob.e_samples[1].front()
                  + r2 * (_pimpl_->tab_prob.e_samples[1].back() - _pimpl_->tab_prob.e_samples[1].front());
  ...
  void dbd_gA::_shoot_e1_e2_inverse_transform_method_(i_random & prng_, double & e1_, double & e2_)
  {
    ...
    if (ie1_found == -1) {
      throw std::logic_error(
          "bxdecay0::dbd_gA::_shoot_inverse_transform_method_: Cound not find the E1 c.d.f. sample!");
    }
    if (debug) {
      std::cerr << "[debug] bxdecay0::dbd_gA::_shoot_inverse_transform_method_: Found the E1 c.d.f. sample #"
                << ie1_found << " < " << _pimpl_->tab_prob.e1_cprobs.size() << std::endl;
      std::cerr.precision(16);
      std::cerr << "[debug] bxdecay0::dbd_gA::_shoot_inverse_transform_method_:  - min prob = "
                << _pimpl_->tab_prob.e1_cprobs[ie1_found - 1] << std::endl;
      std::cerr << "[debug] bxdecay0::dbd_gA::_shoot_inverse_transform_method_:  - max prob = "
                << _pimpl_->tab_prob.e1_cprobs[ie1_found] << std::endl;
    ...
    if (je2_found == -1) {
      throw std::logic_error(
          "bxdecay0::dbd_gA::_shoot_inverse_transform_method_: Cound not find the E2 c.d.f. sample!");
    }
    if (debug) {
      std::cerr << "[debug] bxdecay0::dbd_gA::_shoot_inverse_transform_method_: Found the E2 c.d.f. sample #"
                << je2_found << " < " << sampled_e2cdf.size() << std::endl;
      std::cerr.precision(16);
      std::cerr << "[debug] bxdecay0::dbd_gA::_shoot_inverse_transform_method_:  - min prob = "
                << sampled_e2cdf[je2_found - 1] << std::endl;
      std::cerr << "[debug] bxdecay0::dbd_gA::_shoot_inverse_transform_method_:  - max prob = "
                << sampled_e2cdf[je2_found] << std::endl;
  ...
  // static
  void dbd_gA::export_to_event(i_random & prng_, const double e1_, const double e2_, const double cos12_, event & ev_)
  {
    ...
    // First electron :
    particle.set_momentum(rvp1.x, rvp1.y, rvp1.z);
    ev_.add_particle(particle);
    // Second electron :
    particle.set_momentum(rvp2.x, rvp2.y, rvp2.z);
    ev_.add_particle(particle);
    return;
  }
  ...
  void load_optimized_cdf_array(const std::string & in_str_, std::vector<double> & tab_cprobs_)
  {
    ...
    while (in) {
      ...
      if (token[0] == '^') {
        ...
        din >> digits;
        if (!din) {
          throw std::logic_error("bxdecay0::load_optimized_cdf_array: Format error at token '"
