...
namespace bxdecay0 {
  ...
  void I126(i_random & prng_, event & event_, const double tcnuc_, double & tdnuc_)
  {
    double t;
    double tdlev;
    double p;
    ...
    pdecay = 100. * prng_();
    if (pdecay <= 47.300)
      goto label_1; // 47.300% beta- to 126Xe
    if (pdecay <= 98.992)
      goto label_2; // 51.692% EC to 126Te
  ...
  label_2:
    ...
    pec = 100. * prng_();
    if (pec <= 0.014)
      goto label_2045; // 0.014%
    if (pec <= 0.015)
      goto label_1873; // 0.001%
    if (pec <= 8.630)
      goto label_1420; // 8.615%
    if (pec <= 63.800)
      goto label_666; // 55.170%
    return;           // 36.200%
  label_2045:
    thlev = 0.;
    p     = 100. * prng_();
