...
#else
    // fprintf(stderr, "[devel] _br_find_exe_for_symbol: symbol=%p\n", symbol);
#define SIZE PATH_MAX + 100
    ...
    if (f == NULL)
      return (char *) NULL;
    // fprintf(stderr, "[devel] _br_find_exe_for_symbol: fopen(/proc/self/maps)\n");
	...
	break;
      ...
      /* Sanity check. */
      if ((strstr (line, " r-xp ") == NULL && strstr (line, " r--p ") == NULL) || strchr (line, '/') == NULL)
	continue;
    ...
    free (address_string);
    fclose (f);
    // fprintf(stderr, "[devel] _br_find_exe_for_symbol: found='%s'\n", found);
...
#define NULL ((char *) 0) /* typecasted as char* for C++ type safeness */
  ...
  br_init_lib (BrInitError *error)
  {
    ...
    // fprintf(stderr, "[devel] br_init_lib = '%s'\n", exe);
    return exe != NULL;
  }
